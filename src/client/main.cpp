...
int main(int argc, char **argv)
{
    ...
    // 创建client端的socket
    int clientfd = socket(2, SOCK_STREAM, 0); // AF_INET
    if (-1 == clientfd)
    {
                    ...
                    else // 登录成功
                    {
                        ...
                        if (responsejs.contains("offlinemsg"))
                        {
                            ...
                            for (string &str : vec)
                            {
                                ...
                                if (ONE_CHAT_MSG == js["msgid"].get<int>()) // 个人聊
                                {
                        ...
                        // 登录成功，启动接收线程负责接收数据，该线程只启动一次（重复登录）
...
void showCurrentUserData()
{
    ...
    {
        cout << "groups of " << g_currentUser.getName() << endl;
        for (Group &group : g_currentUserGroupList)
        {
            cout << group.getId() << " " << group.getName() << " " << group.getDesc() << endl;
            for (GroupUser &user : group.getUsers())
            {
                cout << user.getName()<< " " << user.getId() << " " << user.getState()
                     << " " << user.getRole() << endl;
            }
        }
    }
